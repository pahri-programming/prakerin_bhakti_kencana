                ...
                <div class="card border-0 shadow-sm rounded-3">
                    ...
                    <div class="card-body p-4">
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                <label class="text-muted small mb-1">Harga Barang</label>
                                <div class="fw-semibold fs-5 text-success">
                                    {{ $barang->harga > 0 ? 'Rp ' . number_format($barang->harga, 0, ',', '.') : 'Belum diset' }}
                                </div>
                            </div>

                            {{-- Simulasi denda (muncul kalau harga > 0) --}}
                            @if ($barang->harga > 0)
                                <div class="col-12">
                                    <label class="text-muted small mb-1">Simulasi Denda</label>
                                    <div class="d-flex flex-wrap gap-2 mt-1">
                                        <span class="badge bg-warning text-dark px-3 py-2">
                                            <i class="fas fa-exclamation-triangle me-1"></i>
                                            Rusak Ringan (20%): Rp {{ number_format($barang->harga * 0.2, 0, ',', '.') }}
                                        </span>
                                        <span class="badge bg-danger px-3 py-2">
                                            <i class="fas fa-times-circle me-1"></i>
                                            Rusak Berat (80%): Rp {{ number_format($barang->harga * 0.8, 0, ',', '.') }}
                                        </span>
                                        <span class="badge bg-dark px-3 py-2">
                                            <i class="fas fa-ban me-1"></i>
                                            Hilang (100%): Rp {{ number_format($barang->harga, 0, ',', '.') }}
                                        </span>
                                    </div>
                                </div>
                            @endif
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    ...
                    <div class="card-body p-4">
                        <div class="d-grid gap-2">
                            <a href="{{ route('backend.barang.edit', $barang->id) }}" class="btn btn-warning">
