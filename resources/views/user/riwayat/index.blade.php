...
<style>
    ...
    .section-head h6 { margin: 0; font-weight: 700; font-size: .95rem; }
    ...
    /* Table */
    .riwayat-table { width: 100%; border-collapse: collapse; font-size: .875rem; }
    .riwayat-table th {
    ...
    /* Barang list */
    .barang-list { display: flex; flex-wrap: wrap; gap: 4px; }
    .barang-tag {
    ...
    .btn-export:hover { background: #f5b7b1; color: #922b21; }
</style>
@endpush
...
<div class="col-xl-11 col-lg-12">
    ...
    <div class="tab-nav">
        <button class="tab-btn active" onclick="switchTab('booking', this)">
            ...
            <span class="badge bg-primary bg-opacity-20 text-primary ms-1" style="font-size:.7rem;">{{ $booking->count() }}</span>
        </button>
        <button class="tab-btn" onclick="switchTab('peminjaman', this)">
            <i class="fas fa-boxes"></i> Peminjaman Barang
            <span class="badge bg-success bg-opacity-20 text-success ms-1" style="font-size:.7rem;">{{ $peminjaman->count() }}</span>
        </button>
        <button class="tab-btn" onclick="switchTab('denda', this)">
            <i class="fas fa-file-invoice-dollar"></i> Denda
            <span class="badge bg-danger bg-opacity-20 text-danger ms-1" style="font-size:.7rem;">{{ $denda->count() }}</span>
        </button>
    </div>
            ...
            <div class="filter-bar">
                <form method="GET" action="{{ route('riwayat.index') }}" id="form-booking">
                    ...
                    <div class="row g-2 align-items-end">
                        <div class="col-md-3">
                            <label class="form-label" style="font-size:.78rem; color:#64748b; font-weight:600;">Ruangan</label>
                        ...
                        <div class="col-md-3">
                            <label class="form-label" style="font-size:.78rem; color:#64748b; font-weight:600;">Status</label>
                            <select name="status_booking" class="form-select form-select-sm">
                                ...
                                <option value="Diterima"  {{ request('status_booking') == 'Diterima'  ? 'selected' : '' }}>Diterima</option>
                                <option value="Ditolak"   {{ request('status_booking') == 'Ditolak'   ? 'selected' : '' }}>Ditolak</option>
                                <option value="Selesai"   {{ request('status_booking') == 'Selesai'   ? 'selected' : '' }}>Selesai</option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label" style="font-size:.78rem; color:#64748b; font-weight:600;">Tanggal</label>
                                ...
                                @foreach ($booking as $b)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="badge-pill" style="background:#f1f5f9;color:#64748b;">{{ $b->status }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                    ...
                    <div class="row g-2 align-items-end">
                        <div class="col-md-3">
                            ...
                            <select name="ruangan_pinjam_id" class="form-select form-select-sm">
                                <option value="">Semua Ruangan</option>
                                @foreach ($ruangan as $r)
                                    <option value="{{ $r->id }}" {{ request('ruangan_pinjam_id') == $r->id ? 'selected' : '' }}>
                                        {{ $r->nama_ruangan }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-2">
                            <label class="form-label" style="font-size:.78rem; color:#64748b; font-weight:600;">Barang</label>
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="status_peminjaman" class="form-select form-select-sm">
                                <option value="">Semua Status</option>
                                <option value="menunggu"  {{ request('status_peminjaman') == 'menunggu'  ? 'selected' : '' }}>Menunggu</option>
                        ...
                        <div class="col-md-2">
                            <label class="form-label" style="font-size:.78rem; color:#64748b; font-weight:600;">Tgl Pinjam</label>
                            <input type="date" name="tanggal_pinjam" class="form-control form-control-sm"
                                value="{{ request('tanggal_pinjam') }}">
                        </div>
                        <div class="col-md-1">
                            <label class="form-label" style="font-size:.78rem; color:#64748b; font-weight:600;">Tgl Kembali</label>
                        ...
                        <table class="riwayat-table">
                            ...
                            <tbody>
                                @foreach ($peminjaman as $p)
                                    <tr>
                                        <td class="text-muted">{{ $loop->iteration }}</td>
                                        <td><code style="font-size:.8rem;">{{ $p->kode ?? '-' }}</code></td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            @elseif($st === 'selesai')
                                                <span class="badge-pill badge-selesai">Selesai</span>
                                            @else
                                                <span class="badge-pill" style="background:#f1f5f9;color:#64748b;">{{ $p->status }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
    ...
    <div id="tab-denda" class="tab-panel">
        <div class="section-card">
            <div class="section-head">
                <h6><i class="fas fa-file-invoice-dollar me-2 text-danger"></i>Riwayat Denda Peminjaman</h6>
            </div>
            <div class="p-0">
                @if ($denda->isEmpty())
                    <div class="empty-state">
                        <i class="fas fa-check-circle text-success"></i>
                        Tidak ada tagihan denda. Semua peminjaman berjalan baik!
                    </div>
                @else
                    <div class="table-responsive">
                        <table class="riwayat-table">
                            ...
                            <tbody>
                                @foreach ($denda as $d)
                                    <tr>
                                        <td class="text-muted">{{ $loop->iteration }}</td>
                                        <td><code style="font-size:.8rem;">{{ $d->pengembalianBarang->peminjamanBarang->kode ?? '-' }}</code></td>
                                        <td>
                                            @php $kondisi = $d->verifikasiPengembalian->kondisi ?? '-' @endphp
                                            @if($kondisi === 'rusak_ringan')
                                                <span class="badge-pill badge-menunggu">Rusak Ringan</span>
                                            @elseif($kondisi === 'rusak_berat')
                                                <span class="badge-pill badge-ditolak">Rusak Berat</span>
                                            @elseif($kondisi === 'hilang')
                                                <span class="badge-pill" style="background:#1e293b;color:#fff;">Hilang</span>
                                            @else
                                        ...
                                        <td>{{ $d->tanggal_tindakan ? \Carbon\Carbon::parse($d->tanggal_tindakan)->translatedFormat('d F Y') : '-' }}</td>
                                        <td>
                                            @if($d->status_pembayaran === 'belum_bayar')
                                                <span class="badge-pill badge-ditolak">Belum Bayar</span>
                                            @elseif($d->status_pembayaran === 'menunggu_verifikasi')
                                                <span class="badge-pill badge-menunggu">Menunggu Verifikasi</span>
                                            ...
                                            @elseif($d->status_pembayaran === 'dibebaskan')
                                                <span class="badge-pill" style="background:#f1f5f9;color:#64748b;">Dibebaskan</span>
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('user.denda.show', $d->id) }}"
                                                class="btn btn-sm btn-outline-danger" style="font-size:.78rem;">
